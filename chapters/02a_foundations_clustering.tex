...
For continuos data, the proximity between items is typically quantified by dissimilarity in terms of distance measures. The three main distance functions are visualized in figure~\ref{fig:clustering-proximity} and explained as follows~\cite{Meert06clustermaps}. $x$ and $y$ represent the spatial position of a data item that is part of the clustering process, indexed by $i$.
